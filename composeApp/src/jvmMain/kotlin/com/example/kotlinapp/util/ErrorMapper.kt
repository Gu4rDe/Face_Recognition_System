...
fun mapException(e: Exception): String = when (e) {
    is ApiException -> when (e.code) {
        ...
        403 -> {
            val detail = parseDetail(e.message)
            if (detail != null && detail.lowercase().contains("not found")) {
                "$detail\nПроверьте правильность имени пользователя"
            } else {
                "${detail ?: "Доступ запрещён"}\nПроверьте, что ваш код приглашения действителен и не истёк"
            }
        }
        404 -> "${parseDetail(e.message) ?: "Пользователь не найден"}\nПроверьте правильность имени пользователя"
        409 -> "Пользователь уже существует\nПопробуйте другой логин или email"
        422 -> "${parseDetail(e.message) ?: "Некорректные данные"}\nПроверьте правильность заполнения всех полей"
        503 -> "Сервис распознавания лиц недоступен. Попробуйте позже"
